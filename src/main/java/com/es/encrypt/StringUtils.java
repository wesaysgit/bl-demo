...
public class StringUtils {
    ...
    public static String getValue(String privateParameters, String valueName) {
        List<JSONObject> jsonObjects = JSONObject.parseArray(privateParameters, JSONObject.class);
        String value = "";
        for (JSONObject jsonObject : jsonObjects) {
            if (jsonObject.getString("plat_key").equals(valueName)) {
                value = jsonObject.getString("key_value");
            }
        }
        return value;
    }

    public static String getPictureValue(String privateParameters, String valueName) {
        List<JSONObject> jsonObjects = JSONObject.parseArray(privateParameters, JSONObject.class);
        String value = "";
        for (JSONObject jsonObject : jsonObjects) {
            if (jsonObject.getString("plat_key").equals(valueName)) {
                value = jsonObject.getString("picture_src");
            }
        }
        return value;
    }

    public static String setValue(String privateParameters, String valueName, String value) {
        List<JSONObject> jsonObjects = JSONObject.parseArray(privateParameters, JSONObject.class);
        for (JSONObject jsonObject : jsonObjects) {
            if (jsonObject.getString("plat_key").equals(valueName)) {
    ...
    public static BigDecimal getLongValue(String privateParameters, String valueName) {
        List<JSONObject> jsonObjects = JSONObject.parseArray(privateParameters, JSONObject.class);
        BigDecimal value = null;
        for (JSONObject jsonObject : jsonObjects) {
            if (jsonObject.getString("plat_key").equals(valueName)) {
                value = jsonObject.getBigDecimal("key_value");
            }
        }
        return value;
    }
    ...
    public static String createMapJson(Map<String, String> info) {
        String json = "";
        int j = 0;
        if (info != null && info.size() > 0) {
            json += "{";
    ...
    public static BigDecimal getDecimalVal(String val) {
        BigDecimal decimal = BigDecimal.ZERO;
        try {
            decimal = BigDecimal.valueOf(getDoubleValue(val)).setScale(5, BigDecimal.ROUND_DOWN);
        } catch (Exception e) {
    ...
    public static BigDecimal getDecimalVal(String val, int scale) {
        BigDecimal decimal = BigDecimal.ZERO;
        try {
            decimal = BigDecimal.valueOf(getDoubleValue(val)).setScale(scale, BigDecimal.ROUND_DOWN);
        } catch (Exception e) {
            decimal = BigDecimal.ZERO;
            e.printStackTrace();
            return decimal;
